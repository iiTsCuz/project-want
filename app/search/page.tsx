...
export default function SearchPage() {
  ...
  const parsed = useMemo(() => parseWant(query), [query]);
    ...
    async function loadOffers() {
      ...
      try {
        const response = await fetch(
          `/api/search?q=${encodeURIComponent(parsed.product)}`
        );
  ...
  async function activateWant() {
    ...
    if (!user) {
      ...
      setMessage("Sign in first to activate this WANT.");
      return;
    }

    if (!bestOffer) {
      setMessageType("error");
      setMessage("Wait for the search to finish first.");
      return;
    }
    ...
    try {
      ...
      setMessageType("error");
      setMessage(`Could not save WANT: ${errorMessage}`);
    } finally {
      setActivating(false);
    }
  }
            ...
            <div className="mt-4 flex items-center gap-2 text-lg font-medium">
              <span
                className={`h-2 w-2 rounded-full ${
                  activated ? "bg-green-400" : "bg-white/30"
                }`}
              />
        ...
        <section className="mt-8">
          ...
          <div className="grid gap-4">
            {offers.map((offer) => {
              const underTarget =
                parsed.targetPrice !== null &&
                offer.price <= parsed.targetPrice;
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-xl bg-white/[0.06] text-xs text-white/30">
                      {offer.image ? (
                        <img
                          src={offer.image}
                          alt={offer.title}
