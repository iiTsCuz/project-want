...
function parseWant(query: string) {
  ...
  const priceRegex =
    /(?:sotto(?:\s+(?:i|il|la|le|gli))?|under|below|a\s+meno\s+di|meno\s+di|massimo|max|entro|non\s+oltre|fino\s+a|<)\s*€?\s*(\d+(?:[.,]\d+)?)\s*(?:€|euro|eur)?/i;

  const priceMatch =
    normalized.match(priceRegex);
  ...
  cleanedProduct =
    cleanedProduct
      .replace(
        /^(?:io\s+)?(?:voglio|vorrei|cerco|cercami|trovami|trova|mi\s+serve|mi\s+servirebbe|sto\s+cercando)\s+/i,
        ""
      )
      .replace(
        /^(?:un|uno|una|il|lo|la|i|gli|le)\s+/i,
        ""
      )
      .replace(
        /\s+(?:a|per)\s*$/i,
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <div className="mt-2 max-w-3xl text-sm text-white/30">
                {loadingOffers
                  ? "WANT is checking current offers."
                  : bestOffer
